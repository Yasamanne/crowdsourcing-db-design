...
-- -------------------------------
-- Creating Stored procedure to load data from original tables to normalized tables
-- --------------------------------
...
DELIMITER $$
CREATE PROCEDURE load_data_theme_ids()
BEGIN      
       insert into loan_theme_ids_ready (
		loan_id, loan_theme_id, loan_theme_type, partner_id ) 
        select id, loan_theme_id, loan_theme_type, partner_id  
        from  loan_them_ids;
END $$
DELIMITER ;

call load_data_theme_ids();
...
DELIMITER $$
CREATE PROCEDURE load_data_into_category()
BEGIN      
 insert into `kiva`.`category_ready` (
 loan_id,sector,loan_use) select 
 loan_id,sector,loan_use from kiva.loan;
END $$
DELIMITER ;

call load_data_into_category();
...
-- -------------------------------------------------------------
-- Running Simple and Complex SQL Queries --
-- -------------------------------------------------------------
